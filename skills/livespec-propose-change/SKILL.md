...
# livespec-propose-change — pi Driver binding

This file is the thin pi binding for the `propose-change` operation, shipped by the
...
`<core-root>/prose/propose-change.md`.

Order of work, every time:

1. Resolve `<core-root>` (next section).
2. Read `<core-root>/prose/propose-change.md` **completely** with the `read` tool.
...
`spec_clis.propose_change` as an argv-form array, pre-populated with core's reference
default and individually overridable. To "run the propose-change CLI named in config":

1. Read `<project-root>/.livespec.jsonc` (JSONC — tolerate `//` comments). If
   the file, the `spec_clis` section, or the `spec_clis.propose_change` key is
   absent, use core's reference default argv:
   `python3 <core-root>/scripts/bin/propose_change.py`.
...
`propose-change` MUTATES the specification tree. Per the Driver-shipped-hooks
...
- **"run the propose-change CLI named in config" / "invoke the propose-change CLI"** —
...
- **"the revise / critique / seed operation"** — the
  `/skill:livespec-revise`, `/skill:livespec-critique`,
  `/skill:livespec-seed` skills in this Driver package.
